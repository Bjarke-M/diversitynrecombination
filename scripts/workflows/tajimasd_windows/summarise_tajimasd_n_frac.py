...
def gather_tajimasd_files(ref_assembly,window_size, dictionary_of_species): # This function generates a dictionary of files
    dictionary_of_files = {}
    for genus in dictionary_of_species[ref_assembly]:
        for species in dictionary_of_species[ref_assembly][genus]:
            ...
            if genus not in dictionary_of_files: 
                dictionary_of_files[genus] = {species: f'/home/bjarkemp/primatediversity/people/bjarkemp/diversitynrecombination/results/tajimasd/{ref_assembly}/{species}/nonpar/{species}_{window_size}.Tajima.D'}
    ...
    for genus in dictionary_of_species[ref_assembly]:
        for species in dictionary_of_species[ref_assembly][genus]:
            ...
            if species not in dictionary_of_files[genus]:
                dictionary_of_files[genus][species] = f'/home/bjarkemp/primatediversity/people/bjarkemp/diversitynrecombination/results/tajimasd/callable_fraction/{ref_assembly}/{species}/{window_size}_callable_fraction.csv'
    return dictionary_of_files
    ...
    for genus in dictionary_of_species[ref_assembly]:
        for species in dictionary_of_species[ref_assembly][genus]:
            if genus not in dictionary_of_files:
                dictionary_of_files[genus] = {}
            if species not in dictionary_of_files[genus]:
                dictionary_of_files[genus][species] = f'/home/bjarkemp/primatediversity/people/bjarkemp/diversitynrecombination/data/tajimasd/recombinationrates/{ref_assembly}/{species}/nonpar/{species}_{window_size}_recomb.bed'
    return dictionary_of_files
...
def wrapper(ref_assembly, window_size, dictionary_of_species): 
    ...
    tajimasd = load_in_datasets(gather_tajimasd_files(ref_assembly, window_size, dictionary_of_species))
    tajimasd = tajimasd.rename(columns={'CHROM': 'chr', 'BIN_START': 'start'})
    merged = pd.merge(frac_n_recombination, tajimasd, on=['chr', 'start', 'genus','species'])
    return merged
...
# ref_assembly = 'Daubentonia_madagascariensis'
# window_size = 100000
