...
def load_in_distance_dataset(files) -> pd.DataFrame: # This function loads in the datasets
    dfs = []
    for genus in files:
        for species, file in  files[genus].items():
            df = pd.read_csv(file, sep='\t', header=None)
            header=['chr','start','end','protein_chr','protein_start','protein_end','protein_id','distance_to_closest_gene']
            df.columns = header
            df['species'] = species
            df['genus'] = genus
            dfs += [df]
...
def gather_distance_files(ref_assembly,window_size, dictionary_of_species): # This function generates a dictionary of files
    dictionary_of_files = {}
    for genus in dictionary_of_species[ref_assembly]:
        for species in dictionary_of_species[ref_assembly][genus]:
            if genus not in dictionary_of_files:
                dictionary_of_files[genus] = {}
            if species not in dictionary_of_files[genus]:
                dictionary_of_files[genus][species] = f'/home/bjarkemp/primatediversity/people/bjarkemp/diversitynrecombination/data/distance_to_genes/{ref_assembly}/{species}_{window_size}_distance_to_genes.bed'
    return dictionary_of_files
...
def wrapper(ref_assembly, window_size, dictionary_of_species): 
    distance_files = load_in_distance_dataset(gather_distance_files(ref_assembly, window_size, dictionary_of_species))
    callable_frac_files = load_in_datasets(gather_callable_frac_files(ref_assembly, window_size, dictionary_of_species))
    frac_n_recombination = pd.merge(callable_frac_files, distance_files, on=['chr', 'start', 'end', 'genus','species'])
...
#print(load_in_distance_dataset(gather_distance_files(ref_assembly, window_size, dictionary_of_species)))
